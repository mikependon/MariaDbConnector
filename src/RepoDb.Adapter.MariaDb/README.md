<div align="center">
    <a href="https://www.nuget.org/packages/RepoDb.Adapter.MariaDb">
...
RepoDb.Adapter.MariaDb aims to:
...
RepoDb.Adapter.MariaDb is built around the standard abstractions provided by `System.Data.Common`.

The following provider-specific objects form the core of the connector:

| RepoDb.Adapter.MariaDb                 | ADO.NET Base Class          | Purpose                                     |
| -------------------------------- | --------------------------- | ------------------------------------------- |
...
RepoDb.Adapter.MariaDb is intended to provide the familiar ADO.NET programming model.

```csharp
using RepoDb.Adapter.MariaDb;
...
RepoDb.Adapter.MariaDb aims to provide a `MariaDbType` enumeration in addition to the standard ADO.NET `DbType`.
...
`MariaDbProviderFactory` extends `DbProviderFactory` and enables provider-independent ADO.NET applications and libraries to create RepoDb.Adapter.MariaDb objects.
...
RepoDb.Adapter.MariaDb provides bulk-loading support under the `RepoDb.Adapter.MariaDb.Bulk` namespace, built on top of `LOAD DATA LOCAL INFILE` via `MySql.Data`'s `MySqlBulkLoader`.

| RepoDb.Adapter.MariaDb.Bulk                       | Purpose                                                                             |
| -------------------------------------------- | ------------------------------------------------------------------------------------ |
...
RepoDb.Adapter.MariaDb is more than a set of ADO.NET wrapper classes. The public ADO.NET API will sit on top of the MariaDB communication and protocol infrastructure.
...
Although RepoDb.Adapter.MariaDb can be used directly through ADO.NET, it is designed to work naturally with libraries that operate against the standard `System.Data.Common` abstractions.
...
RepoDb.Adapter.MariaDb is in its early stages, and contributions are welcome.
