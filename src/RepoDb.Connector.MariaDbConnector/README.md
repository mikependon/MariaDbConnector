<div align="center">
    <a href="https://www.nuget.org/packages/RepoDb.Connector.MariaDbConnector">
        <image src="logo.png" style="width:256px;" />
    </a>
    <br/>
    <span style="font-size:16px;font-weight:bold;">A lightweight, modern, and open-source ADO.NET data provider for MariaDB, built for RepoDB.</span>
</div>

-----

> **Disclaimer**: This is an independent, unofficial .NET provider for MariaDB. It is a thin ADO.NET wrapper and type-mapping layer built on top of [MySqlConnector](https://www.nuget.org/packages/MySqlConnector) and is not affiliated with or endorsed by MariaDB plc or the MariaDB Foundation.
...
It implements the `Async` equivalent of the [MySqlConnector](https://www.nuget.org/packages/MySqlConnector) library that is dedicated for MariaDB. It also covers the full implementation of Bulk operations using `MySqlConnector`'s `MySqlBulkCopy` class.

> This connector is the `MySqlConnector`-based counterpart to [RepoDb.Connector.MariaDb](https://www.nuget.org/packages/RepoDb.Connector.MariaDb), which is built on Oracle's `MySql.Data` instead. Both packages expose the same `MariaDb`-prefixed API; pick whichever underlying driver best fits your project.
...
RepoDb.Connector.MariaDbConnector aims to:
...
RepoDb.Connector.MariaDbConnector is built around the standard abstractions provided by `System.Data.Common`.

The following provider-specific objects form the core of the connector:

| RepoDb.Connector.MariaDbConnector                 | ADO.NET Base Class          | Purpose                                     |
| -------------------------------- | --------------------------- | ------------------------------------------- |
...
RepoDb.Connector.MariaDbConnector is intended to provide the familiar ADO.NET programming model.
...
RepoDb.Connector.MariaDbConnector aims to provide a `MariaDbType` enumeration in addition to the standard ADO.NET `DbType`.
...
`MariaDbTypeConverter` provides the current leg of that mapping, converting between `MariaDbType` and the underlying `MySqlConnector.MySqlDbType`:
...
`MariaDbProviderFactory` extends `DbProviderFactory` and enables provider-independent ADO.NET applications and libraries to create RepoDb.Connector.MariaDbConnector objects.
...
RepoDb.Connector.MariaDbConnector provides bulk-loading support under the `RepoDb.Connector.MariaDbConnector.Bulk` namespace, built directly on top of `MySqlConnector`'s `MySqlBulkCopy` class.

| RepoDb.Connector.MariaDbConnector.Bulk                       | Purpose                                                                             |
| -------------------------------------------- | ------------------------------------------------------------------------------------ |
...
`MariaDbBulkCopy` wraps a `MySqlConnector.MySqlBulkCopy` internally, resolving any name-based column mapping (source column name, destination column ordinal) against the source schema and the destination table's `SHOW COLUMNS` output before delegating the actual write:
...
> Unlike [RepoDb.Connector.MariaDb](https://www.nuget.org/packages/RepoDb.Connector.MariaDb) (whose `MariaDbBulkCopy` streams through a temporary `LOAD DATA LOCAL INFILE` file via `MariaDbBulkLoader`), this package has no `BatchSize` property or `MariaDbBulkLoader` class — `MySqlBulkCopy` writes over the wire protocol directly and has no batching concept to expose.

## Architecture

RepoDb.Connector.MariaDbConnector is more than a set of ADO.NET wrapper classes. The public ADO.NET API will sit on top of the MariaDB communication and protocol infrastructure.
...
11. `MariaDbBulkCopy`, `MariaDbBulkColumnMapping`, and `MariaDbBulkCopyColumnMappingCollection`
...
Although RepoDb.Connector.MariaDbConnector can be used directly through ADO.NET, it is designed to work naturally with libraries that operate against the standard `System.Data.Common` abstractions.
...
Other DbConnection-based Libraries
          │
          ▼
   RepoDb.Connector.MariaDbConnector
          │
          ▼
      MariaDB Server
```
...
RepoDb.Connector.MariaDbConnector is in its early stages, and contributions are welcome.
